/* This file copied from Tk 8.4.19 and modified to continue support the now
   deprecated TK_CONFIG_OPTION_SPECIFIED flag. */
/* 
 * tkOldConfig.c --
 *
 ...
 *
 * RCS: @(#) $Id: bltOldConfig.c,v 1.3 2009/10/25 04:30:52 pcmacdon Exp $
 */
...
int
Blt_ConfigureWidget(interp, tkwin, origSpecs, argc, argv, widgRec, flags)
...
int
Blt_ConfigureInfo(interp, tkwin, specs, widgRec, argvName, flags)
    Tcl_Interp *interp;		/* Interpreter for error reporting. */
					 ...
					 * values of info for widget. */
{
    ...
    if (strstr(Tk_PathName(tkwin), ".__##") &&
        specPtr->type < TK_CONFIG_CUSTOM && specPtr->type >=0) {
        static char *conftypes[TK_CONFIG_END+10] = {
            "bool", "int", "double", "string", "uid", "color", "font",
                "bitmap", "border", "relief", "cursor", "activecursor", "justify",
                "anchor", "syn", "cap", "join", "pixels", "mm", "window", "custom",
                "END"
        };
        if (conftypes[TK_CONFIG_END] == 0 ||
        strcmp(conftypes[TK_CONFIG_END],"END")) {
            fprintf(stderr, "Tk_ConfigTypes in blt changed\n");
        }
				 ...
				 * if result is static. */
{
    ...
    *freeProcPtr = NULL;
	...
	case TK_CONFIG_WINDOW: {
	    ...
	    ntkwin = *((Tk_Window *) ptr);
	    if (ntkwin != NULL) {
		result = Tk_PathName(ntkwin);
	    }
	    break;
	}
	case TK_CONFIG_CUSTOM:
...
int
Blt_ConfigureValue(interp, tkwin, specs, widgRec, argvName, flags)
    Tcl_Interp *interp;		/* Interpreter for error reporting. */
    Tk_Window tkwin;		/* Window corresponding to widgRec. */
    Tk_ConfigSpec *specs;	/* Describes legal options. */
    char *widgRec;		/* Record whose fields contain current
 ...
 */
	...
	/* ARGSUSED */
void
Blt_FreeOptions(specs, widgRec, display, needFlags)
    Tk_ConfigSpec *specs;	/* Describes legal options. */
    char *widgRec;		/* Record whose fields contain current
				 * values for options. */
...
Tk_ConfigSpec *
Blt_GetCachedSpecs(interp, staticSpecs)
Tcl_Interp *interp;
const Tk_ConfigSpec *staticSpecs;
{
    return GetCachedSpecs(interp, staticSpecs);
}
