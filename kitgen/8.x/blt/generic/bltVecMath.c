...
typedef double (Opt1Proc) _ANSI_ARGS_((VectorObject *vPtr, VectorObject *optPtr));
...
typedef struct {
    ...
    int funcArg;    /* This value is to be a function argument. */
    int isLast;
} Value;
...
typedef struct {
    ...
    VectorObject *vPtr;  /* Allow functions to use no arguments for self. */
} ParseInfo;
...
static int NextValue _ANSI_ARGS_((Tcl_Interp *interp, ParseInfo *parsePtr,
	int prec, Value * valuePtr, Value * optValPtr));
    ...
    VectorObject *vPtr;
{
    ...
    v = vPtr->valueArr;
    for(i = vPtr->first, j = vPtr->last ; i < j; i++, j--) {
        t = v[i];
        v[i] = v[j];
        v[j] = t;
    }
...
{
    VectorObject *vPtr = (VectorObject *)vecPtr;
    VectorObject *oPtr = (VectorObject *)optPtr;
    double *v, def = 0;
    int i, offs, ni;
    v = vPtr->valueArr;
    if (oPtr==NULL || oPtr->length<1 || !v) { return 0; }
    offs = oPtr->valueArr[0];
    if (offs == 0) return 0;
    if (oPtr->length>1) {
        def = oPtr->valueArr[1];
    }
    if (offs>0) {
        for (i=vPtr->length-1; i>=0; i--) {
            ni = i-offs;
            if (ni<0 || ni>=vPtr->length) {
                v[i] = def;
            } else {
                v[i] = v[ni];
            }
				 ...
				 * correctly. */
{
    ...
    *p = '(';
    ...
    optVal.funcArg = mathPtr->hasArg;
    if (NextValue(interp, parsePtr, -1, valuePtr, mathPtr->hasArg?&optVal:NULL) != TCL_OK) {
        /* Ok to skip argument and use self. */
        if (valuePtr->funcArg == -1 && parsePtr->vPtr && !mathPtr->hasArg) {
            Blt_VectorDuplicate(vPtr=valuePtr->vPtr, parsePtr->vPtr);
            Tcl_ResetResult(interp);
            goto callfunc;
        }
	...
	result = TCL_ERROR;	/* Missing right parenthesis */
	goto cleanup;
    }
    callfunc:
    if ((*mathPtr->proc) (mathPtr->clientData, interp, vPtr, optVal.vPtr)
	!= TCL_OK) {
	result = TCL_ERROR;	/* Function invocation error */
     } else {
				 ...
				 * correctly. */
{
    ...
    if ((*p == 'l') && (p[1] == 'a') && (p[2] == 's') && (p[3] == 't') && (p[4] == 0)) {

        parsePtr->token = VALUE;
        parsePtr->nextPtr = p+4;
        if (Blt_VectorChangeLength(valuePtr->vPtr, 1) != TCL_OK) {
            return TCL_ERROR;
        }
        valuePtr->vPtr->valueArr[0] = 0;
        valuePtr->isLast = 1;
        return TCL_OK;
    }
    parsePtr->nextPtr = p + 1;
 ...
 */
static int
NextValue(interp, parsePtr, prec, valuePtr, optValPtr)
    Tcl_Interp *interp;		/* Interpreter to use for error reporting. */
    ParseInfo *parsePtr;	/* Describes the state of the parse
    ...
    Value *optValPtr;
{
    ...
    if (optValPtr) {
        *optValPtr = value2;
        optValPtr->funcArg = 1;
        optValPtr->vPtr = Blt_VectorNew(vPtr->dataPtr);
	...
	result = NextValue(interp, parsePtr, -1, valuePtr, 0);
	if (result != TCL_OK) {
	    goto done;
	}
	if (parsePtr->token != CLOSE_PAREN) {
	...
	if (parsePtr->token >= UNARY_MINUS) {
	    ...
	    result = NextValue(interp, parsePtr, precTable[operator], valuePtr, 0);
	    if (result != TCL_OK) {
		goto done;
	    }
	    gotOp = TRUE;
	...
	if (optValPtr) {
	    if (parsePtr->token != COMMA) {
	        goto error;
	    }
            result = NextToken(interp, parsePtr, optValPtr);
            if (result != TCL_OK) {
                goto error;
            }
            result = NextToken(interp, parsePtr, optValPtr);
	...
	result = NextValue(interp, parsePtr, precTable[operator], &value2, 0);
	if (result != TCL_OK) {
	    goto done;
	}
	...
	if (v2Ptr->length == 1) {
	    ...
	    opnd = vPtr->valueArr;
#define DOLAST if (value2.isLast == 1) scalar = opnd[i];
	    switch (operator) {
	    case MULT:
		for(i = 0; i < vPtr->length; i++) {
		    opnd[i] *= scalar;
 ...
 */
static int
EvaluateExpression(interp, string, valuePtr, cvPtr)
    ...
    VectorObject *cvPtr;
{
    ...
    result = NextValue(interp, &info, -1, valuePtr, 0);
    if (result != TCL_OK) {
	return result;
    }
    if (info.token != END) {
...
static int
Opt1Func(clientData, interp, vPtr, optPtr)
    ClientData clientData;
    Tcl_Interp *interp;		/* Not used. */
    VectorObject *vPtr;
    VectorObject *optPtr;
{
    Opt1Proc *procPtr = (Opt1Proc *) clientData;

    return (*procPtr) (vPtr, optPtr);
}
...
static MathFunction mathFunctions[] =
{
    ...
    {"invert", (GenericMathProc *) VectorFunc, (ClientData)Invert},
    ...
    {"row", (GenericMathProc *) VectorFunc, (ClientData)Row},
    {"sdev", (GenericMathProc *) ScalarFunc, (ClientData)StdDeviation},
    {"shift", (GenericMathProc *) Opt1Func, (ClientData)Shift, 1 },
...
/* Evaluate expression and handle ? : conditional. */
/* TODO: evaluate/compile all expressions to detect syntax error. */
static VectorObject*
EvalVectorExpr(interp, string, dataPtr, vPtr)
    Tcl_Interp *interp;		/* Context in which to evaluate the
				 * expression. */
    char *string;		/* Expression to evaluate. */
    VectorInterpData *dataPtr;	/* Interpreter-specific data. */
    VectorObject *vPtr;
{
    ...
    int i, sz, vm[3], vmax, vmi, isconst, lvl;
    Value values[3];
    double *v[3];
    ...
    value.funcArg = 0;
    quest = strchr(string, '?');
    if (quest && ((colon = strchr(string, ':')))) {
        /* Handling conditional "X ? Y : Z" */
    } else {
        value.vPtr = Blt_VectorNew(dataPtr);
        if (EvaluateExpression(interp, string, &value, vPtr) != TCL_OK) {
    ...
    /* Make a static copy of index. */
    sz = strlen(string);
    if (sz>=200) {
        char *zBuf = Blt_Malloc(sz+1);
        strcpy(zBuf, string);
        string = zBuf;
    } else {
    ...
    if (lvl || *cp != ':') {
        Tcl_AppendResult(interp, "'?' conditional missing ':'", 0);
        return NULL;
    }
    colon = cp;
    *quest = 0;
    *colon = 0;
    ...
    for (i=0; i<3; i++) {
        ...
        if (!*s[i]) {
            Tcl_AppendResult(interp, "null expression in conditional", 0);
           return NULL;
       }
    }
    values[0].vPtr = values[1].vPtr = values[2].vPtr = NULL;
    vmax =0;
    vmi = -1;
    isconst = 1;
    for (i=0; i<3; i++) {
        if ((result = EvalVectorExpr(interp, s[i], dataPtr, vPtr)) == NULL) {
            goto cleanup;
        }
        values[i].vPtr = result;
        v[i] = values[i].vPtr->valueArr;
        vm[i] = values[i].vPtr->length;
        if (vm[i]>vmax) { vmax = vm[vmi=i]; }
        if (i==0) {
            if (values[0].vPtr->length == 1) {
                isconst = 1;
                if (!v[0][0]) { isconst++; i++; }
            }
        } else if (isconst && i==1) break;
    }
    if (isconst==1) {
        result = values[1].vPtr;
        values[1].vPtr = NULL;
    } else if (isconst==2) {
        result = values[2].vPtr;
        values[2].vPtr = NULL;
    } else {
#define VM(n,i) v[n][vm[n]>i?i:vm[n]]
        values[vmi].vPtr = NULL;
        for (i=0; i<result->length; i++) {
            result->valueArr[i] = (VM(0,i) ? VM(1,i) : VM(2,i) );    
        }
        result = values[vmi].vPtr;
    }

    cleanup:
    for (i=0; i<3; i++) {
        if (values[i].vPtr) {
            Blt_VectorFree(values[i].vPtr); 
        }   
    }
    if (string != zStatic) {
        Blt_Free(string);
...
{
    VectorInterpData *dataPtr;	/* Interpreter-specific data. */
    VectorObject *nvPtr, *vPtr = (VectorObject *)vecPtr;
    int result = TCL_OK;
 
    dataPtr = (vecPtr != NULL) 
        ? vPtr->dataPtr : Blt_VectorGetInterpData(interp);

    if ((nvPtr = EvalVectorExpr(interp, string, dataPtr, vPtr)) == NULL) {
        result = TCL_ERROR;
        goto cleanup;
    }

    //printf("VMAX: %d, vmi=%d,  %d,%d,%d\n", vmax, vmi, vm[0], vm[1],vm[2]);
#define VM(n,i) v[n][vm[n]>i?i:vm[n]]
