...
apk update
apk add haveged openssh iw wpa_supplicant dhcpcd dnsmasq hostapd iptables avahi dbus dcron chrony gpsd libgfortran musl-dev fftw-dev libconfig-dev curl wget less nano bc dos2unix
...
zip -r red-pitaya-alpine-3.9-armv7-`date +%Y%m%d`-ft8.zip apps boot.bin cache devicetree.dtb modloop red-pitaya.apkovl.tar.gz start.sh uEnv.txt uImage uInitrd wifi
