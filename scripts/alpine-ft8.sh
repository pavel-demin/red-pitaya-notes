...
tools_tar=apk-tools-static-2.7.6-r0.apk
...
apk update
apk add openssh iw wpa_supplicant dhcpcd dnsmasq hostapd iptables avahi dcron chrony gpsd-timepps libgfortran musl-dev fftw-dev libconfig-dev curl wget less nano bc
...
sed -i '5,6d' apps/sdr_transceiver_ft8/start.sh
rm -f apps/sdr_transceiver_ft8/index.html
cp apps/sdr_transceiver_ft8/start.sh .
...
zip -r red-pitaya-alpine-3.6-armhf-`date +%Y%m%d`-ft8.zip apks apps boot.bin cache devicetree.dtb modloop red-pitaya.apkovl.tar.gz start.sh uEnv.txt uImage uInitrd wifi
