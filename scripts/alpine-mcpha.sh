...
apk update
apk add openssh iw wpa_supplicant dhcpcd dnsmasq hostapd iptables avahi dbus dcron chrony gpsd-timepps musl-dev curl wget less nano bc
...
zip -r red-pitaya-alpine-3.8-armhf-`date +%Y%m%d`-mcpha.zip apks apps boot.bin cache devicetree.dtb modloop red-pitaya.apkovl.tar.gz start.sh uEnv.txt uImage uInitrd wifi
