...
class sdr_transceiver_rx(gr.sync_block):
  ...
  def __init__(self, addr, port, rx_rate, rx_freq, tx_freq, corr):
    ...
    if rx_rate in sdr_transceiver_rx.rx_rates:
      self.set_rate(sdr_transceiver_rx.rx_rates[rx_rate])
    else:
  ...
  def set_rx_rate(self, ratecode):
    self.sock.send(struct.pack('<I', 1<<28 | ratecode))
  ...
  def work(self, input_items, output_items):
    data = self.sock.recv(2048, socket.MSG_WAITALL)
...
class sdr_transceiver_tx(gr.sync_block):
  ...
  def work(self, input_items, output_items):
    self.sock.send(input_items.tostring(), socket.MSG_NOSIGNAL)
