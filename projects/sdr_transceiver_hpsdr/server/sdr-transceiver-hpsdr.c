...
void process_ep2(uint8_t *frame)
{
  ...
  uint8_t cw_internal, cw_volume, cw_delay, preamp;
  ...
  switch(frame[0])
  {
    ...
    case 1:
      ...
      /* set output pins */
      preamp = (*gpio_in & 1) ? 0 : (frame[3] & 0x04) >> 2 | (rx_att_data == 0);
      *gpio_out = (frame[2] & 0x1e) << 3 | (frame[3] & 0x03) << 2 | preamp << 1 | (frame[0] & 0x01);
