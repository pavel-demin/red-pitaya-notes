...
Fc <- 0.0190                   # cutoff frequency
htbw <- 0.0055                 # half transition bandwidth
...
plot(f, 20*log10(Mf), type = "b", ylab = "dB", xlab = "Frequency", xlim = c(0.058, 0.062), ylim = c(-125, 5))
lines(fh$f / pi, 20*log10(abs(fh$h)), col = "blue")
grid()

plot(f, 20*log10(Mf), type = "b", ylab = "dB", xlab = "Frequency", xlim = c(0.018, 0.022), ylim = c(-0.5, 0.1))
lines(fh$f / pi, 20*log10(abs(fh$h)), col = "blue")
grid()
