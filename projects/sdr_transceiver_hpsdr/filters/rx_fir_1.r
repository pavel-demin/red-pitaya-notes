...
Fo <- 0.044                    # Pass band edge; 0.22/5

# fir2 parameters
k <- kaiserord(c(Fo, Fo+0.036), c(1, 0), 1/(2^16), 1)
L <- k$n                       # Filter order
...
fs <- seq(Fo+0.036, 0.5, by=s) # Stop band frequency samples
...
plot(f, Mf, type = "b", ylab = "magnitude", xlab = "Frequency", xlim = c(0, 0.20))
lines(fh$f / pi, abs(fh$h), col = "blue")

plot(f, 20*log10(Mf+1e-5), type = "b", ylab = "dB", xlab = "Frequency", xlim = c(0, 0.20))
