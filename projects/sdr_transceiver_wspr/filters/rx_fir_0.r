...
Fc <- 0.117                    # cutoff frequency
htbw <- 0.05                   # half transition bandwidth
...
plot(f, 20*log10(Mf), type = "b", ylab = "dB", xlab = "Frequency", xlim = c(0.42, 0.44), ylim = c(-125, 5))
lines(fh$f / pi, 20*log10(abs(fh$h)), col = "blue")
grid()

plot(f, 20*log10(Mf), type = "b", ylab = "dB", xlab = "Frequency", xlim = c(0.06, 0.08), ylim = c(-0.5, 0.1))
lines(fh$f / pi, 20*log10(abs(fh$h)), col = "blue")
grid()
