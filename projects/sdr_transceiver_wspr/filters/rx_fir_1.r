...
Fc <- 0.122                    # cutoff frequency
htbw <- 0.004                  # half transition bandwidth
...
Mp[-1] <- abs(M*R*sin(pi*fp[-1]*3/4/R)/sin(pi*M*fp[-1]*3/4))^N
...
plot(f, 20*log10(Mf), type = "b", ylab = "dB", xlab = "Frequency", xlim = c(0.25, 0.27), ylim = c(-125, 5))
lines(fh$f / pi, 20*log10(abs(fh$h)), col = "blue")
grid()

plot(f, 20*log10(Mf), type = "b", ylab = "dB", xlab = "Frequency", xlim = c(0.23, 0.25), ylim = c(-5, 5))
lines(fh$f / pi, 20*log10(abs(fh$h)), col = "blue")
grid()
