...
using namespace ExtIO_RedPitaya_122_88;
...
bool EXTIO_API InitHW(char *name, char *model, int &type)
{
  ...
  strcpy(name, "Red Pitaya SDR 122.88");
  strcpy(model, "");

  if(!gInitHW)
  {
    ManagedGlobals::gKey = Registry::CurrentUser->OpenSubKey("Software\\ExtIO_RedPitaya_122_88", true);
    if(!ManagedGlobals::gKey)
    {
      ManagedGlobals::gKey = Registry::CurrentUser->CreateSubKey("Software\\ExtIO_RedPitaya_122_88");
